...
void main (void) {
  ...
  if(u_enableObjectTexture)
  {
  	...
  	texCoords.s = texCoords.s + sin(texCoords.t*3.14 + u_time*0.01) * 0.1;
    //TASK 2: integrate texture color into phong shader
	//u_material.diffuse = textureColor;
	textureColor = texture2D(u_tex, texCoords);
		//TASK 1: simple texturing: replace vec4(0,0,0,1) with texture lookup
    //gl_FragColor =  texture2D(u_tex, v_texCoords); //replace me for TASK 1 and remove me for TASK 2!!!
