...
function initInteraction(canvas) {
  ...
  canvas.addEventListener('mousemove', function(event) {
    ...
    if (mouse.leftButtonDown) {
      camera.rotation.x += delta.x * 0.5; 
      camera.rotation.y += delta.y * 0.5;
    }
    mouse.pos = pos;
  });
...
function render(timeInMilliseconds) {
  ...
  //context.sceneMatrix = mat4.multiply(mat4.create(), glm.rotateY(camera.rotation.y), glm.rotateX(camera.rotation.X));
